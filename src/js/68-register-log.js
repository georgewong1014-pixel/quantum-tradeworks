...
/* WHAT AN ENTITY IS, IN ONE PLACE.
   ---------------------------------------------------------------------------
   Undo and replay used to be two switch statements listing the same entity
   names, and adding a third entity meant remembering both. The demand register
   was added and neither was touched, which produced a failure worth describing
   because it is the kind that looks like nothing:

   setDemand logged a 'demand' event. lastUndoableEvent found it and reported
   that undo was available. undoLastRegisterChange did not recognise the entity,
   returned null, and — this is the part that mattered — never wrote a reversal.
   So the event stayed the newest un-reversed entry forever. The Undo button
   stayed enabled, did nothing every time it was pressed, said "Nothing left to
   undo", and no earlier change could ever be reached again. One demand record
   permanently disabled undo for the whole register.

   Two changes stop it recurring. Entities are declared here, once, with both
   their undo and their replay; and lastUndoableEvent SKIPS an entity it has no
   handler for instead of stopping on it, so an unregistered entity degrades to
   "not undoable" rather than jamming everything behind it. */
const REGISTER_ENTITIES = {
  observation: {
    ...
    replay(acc, e) {
      if (e.op === 'add') acc.obs.set(e.id, { ...e.after });
      else if (e.op === 'delete') acc.obs.delete(e.id);
      else if (e.op === 'edit' && acc.obs.has(e.id)) acc.obs.set(e.id, { ...acc.obs.get(e.id), [e.field]: e.to });
    },
  },
  ...
  areaAttr: {
    ...
    replay(acc, e) {
      const cur = { ...(acc.areas[e.id] || {}) };
      if (e.to) cur[e.field] = e.to; else delete cur[e.field];
      if (Object.keys(cur).length) acc.areas[e.id] = cur; else delete acc.areas[e.id];
    },
  },

  demand: {
    undo(e) {
      const prev = { ...(State.demand[e.id] || {}) };
      if (e.from) prev[e.field] = e.from; else delete prev[e.field];
      if (Object.keys(prev).length) State.demand[e.id] = prev;
      else delete State.demand[e.id];
      saveDemand();
      logRegister('demand', e.from ? 'edit' : 'delete', e.id,
        { field: e.field, from: e.to, to: e.from, undoOf: e.seq });
      return 'Undid: demand source changed';
    },
    replay(acc, e) {
      const cur = { ...(acc.demand[e.id] || {}) };
      if (e.to) cur[e.field] = e.to; else delete cur[e.field];
      if (Object.keys(cur).length) acc.demand[e.id] = cur; else delete acc.demand[e.id];
    },
  },
};

/* The newest event that has not itself been reversed, is not a reversal, and
   belongs to an entity this file knows how to reverse. That last clause is what
   stops one unhandled entity burying every change behind it. */
...
/* Rebuilds every projection from the log alone. This is what makes the claim
   ...
   Driven by REGISTER_ENTITIES, so an entity added there is replayed here
   without a second edit — which is precisely what did not happen when the
   demand register arrived. */
